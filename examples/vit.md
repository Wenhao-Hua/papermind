...
> **读图**：ViT微调时对预训练1D位置嵌入进行2D插值以适配高分辨率
>
> - 预训练阶段：固定patch大小，图像224×224得N个patch
> - 微调阶段：更高分辨率图像产生更多patch，序列变长
> - 2D插值：将预训练1D嵌入重采样到新2D网格上
> - 双线性插值公式用于计算新网格点的嵌入值
>
> **关键**：位置嵌入的2D插值是ViT适应不同分辨率的唯一2D结构信息注入点
...
>
> - 输入图像分块为patch，嵌入为xi。
> - 随机遮挡50%的patch，80%替换为[mask]。
> - Transformer编码器输出hi，经MLP做512分类。
> - 训练目标：仅对遮挡patch最小化交叉熵损失。
>
> **关键**：通过预测颜色学习视觉表示，无需人工标注。
...
> **读图**：ViT中可学习[class] token的插入与作用机制
>
> - 输入序列z₀由[class] token和196个patch嵌入组成
> - Transformer编码器输出z_L⁰作为图像整体表示
> - 分类头对LN(z_L⁰)进行预测得到类别
>
> **关键**：[class] token输出经LN后用于分类，类似BERT的[CLS]
...
![教学示意图：可学习的1D位置嵌入（Position Embeddings）](figures/vit-fig4.svg)
*教学示意图：可学习的1D位置嵌入（Position Embeddings）（教学示意图）*

> **读图**：ViT使用可学习的1D位置嵌入，无2D空间结构。
>
> - Epos: (N+1)×D的可学习位置嵌入矩阵。
> - 位置0给[class] token，1…N给N个patch。
> - 1D顺序：从左到右、从上到下，无2D网格。
> - 所有嵌入随机初始化，可训练。
>
> **关键**：1D位置嵌入直接加到patch嵌入，无需2D偏置。
