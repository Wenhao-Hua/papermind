...
def _reading_time_minutes(report: Report) -> int:
    """Estimate minutes needed to read this report at ~200 words/minute."""
    texts: List[str] = []
    ...
    for pt in report.technical.details:
        texts += [pt.name, pt.explanation, pt.analogy or ""]
...
def _meta_line(report: Report) -> str:
    ...
    bits.append(f"**阅读时长:** ~{_reading_time_minutes(report)} 分钟")
    return "  •  ".join(bits)
