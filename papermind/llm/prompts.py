...
def svg_figure_user(name: str, explanation: str, formula: Optional[str], context: str) -> str:
    ...
    return (
        ...
        f"【要画的技术点】{name}\n{explanation}{f}\n\n"
        "请按论文正文 Figure 的克制风格直接输出单个合法、完整闭合的 <svg xmlns=\"…\">…</svg>：以核心定义/机制为视觉焦点（加框、最大字号、唯一强调色）；用记号**显式画出**关键集合/映射/匹配（{…}、f: A &#8594; B 小表、二部图连边、小矩阵）；并在关键步骤/元素旁用**简短标注**讲清「它在做什么、量的含义、关键变换」，让读者看图就理解论文这个点——但不要成段散文或单独的\"动机/原理\"框；箭头 ≤ 3 种各表一义并配图例、头部精确落框；忠于本论文记号、不画无关模板；上下标用 `<tspan baseline-shift>`、不用 HTML 标签；所有文字夹在框内不重叠不溢出；合法 XML（& 写成 &amp;、符号用数字实体如 &#215;），以完整 </svg> 收尾。"
    )
