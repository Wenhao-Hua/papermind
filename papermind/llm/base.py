...
class LLMClient:
    ...
    def _record_usage(self, model: str, prompt_tokens: int, completion_tokens: int) -> None:
        cost = 0.0
        # Use litellm only if it is already imported — avoids triggering a first-time
        # Rust/pyo3 extension load from background threads, which can panic and deadlock.
...
# Cached litellm module reference + lock — prevents concurrent first-time imports
# from triggering pyo3 Rust-extension panics in background threads.
_litellm_module: Optional[object] = None
