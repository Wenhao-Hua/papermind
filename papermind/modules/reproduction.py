...
def run(parsed: ParsedPaper, client: LLMClient, context: str) -> Reproduction:
    # Find the real repo FIRST so the guide is grounded in it — otherwise the model
    # invents its own clone steps (e.g. "use a community impl") that then contradict
    # the verified repo we attach below.
    from papermind.repro.repo import find_and_inspect_repo

    repo = find_and_inspect_repo(parsed)
    data = client.complete_json(
        ANALYSIS_SYSTEM, reproduction_user(context, repo_url=repo.url if repo is not None else None)
    )
    ...
    # commands) beats the model's guess. Reuse the repo found above — re-finding it
    # here would repeat the GitHub/PapersWithCode round-trips for no gain.
    if repo is not None:
