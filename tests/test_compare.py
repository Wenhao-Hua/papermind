...
def test_comparison_csv_export():
    ...
    assert r0["year"] == "2023"
    assert r0["main_contribution"] == "faster attention"
    assert "Tiling" in r0["methods"]

    r1 = rows[1]
    assert r1["arxiv_id"] == "1706.03762"
    assert r1["title"] == "Transformer"

    # CSV round-trip via Comparison.to_csv()
