...
def test_comparison_csv_with_synthesis():
    ...
    comp = Comparison(
        papers=[
            ComparedPaper(arxiv_id="A", title="Paper A", main_contribution="contrib A"),
            ComparedPaper(arxiv_id="B", title="Paper B", main_contribution="contrib B"),
        ],
        synthesis="A is better than B in efficiency.",
    )
    text = to_csv(comp)
    rows = list(csv.reader(io.StringIO(text)))
    # synthesis row appears after a blank row
    non_empty = [r for r in rows if r]
    synth_row = next((r for r in non_empty if r[0] == "对比小结"), None)
    assert synth_row is not None
    assert synth_row[1] == "A is better than B in efficiency."
