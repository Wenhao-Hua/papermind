...
def test_comparison_csv_structure():
    ...
    rows = list(reader)
    # header row: 维度 + one col per paper
    assert rows[0] == ["维度", "2307.08691", "1706.03762"]
    # every data row has 3 columns
    for row in rows[1:]:
        assert len(row) == 3
    # check key dimension rows are present
...
def test_comparison_csv_with_synthesis():
    comp = _comparison()
    comp.synthesis = "Paper A is faster; Paper B is the classic."
    text = to_csv(comp)
    assert "对比小结" in text
