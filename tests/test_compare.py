...
def test_comparison_csv_no_pipe_escaping():
    comp = _comparison()
    text = to_csv(comp)
    # CSV must not contain markdown pipe-escaping artifacts
    assert "\\|" not in text
...
def test_comparison_csv_write_file(tmp_path):
    comp = _comparison()
