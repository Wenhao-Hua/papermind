...
def test_comparison_csv_columns_and_rows():
    ...
    # Header row should list all field names
    assert lines[0] == "arxiv_id,title,year,main_contribution,novelty,methods,benchmark,hardware,official_code"
    # Two data rows, one per paper
    assert lines[1].startswith("2307.08691,")
    assert lines[2].startswith("1706.03762,")
    # benchmark value included
    assert "seq=2k: 2.0x" in lines[1]
    # official code included
    assert "https://github.com/x/y" in lines[1]
...
def test_comparison_csv_synthesis_appended():
    comp = _comparison()
    comp.synthesis = "Paper A extends Paper B."
    csv_text = to_csv(comp)
    assert "synthesis" in csv_text
    assert "Paper A extends Paper B." in csv_text
