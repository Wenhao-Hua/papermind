...
def test_comparison_csv_header_and_rows():
    ...
    assert "标题" in labels and "核心贡献" in labels and "关键方法" in labels
...
def test_comparison_csv_synthesis_appended():
    ...
    assert synth_rows and "Both papers improve attention efficiency." in synth_rows[0][1]
...
def test_comparison_csv_via_schema_method(tmp_path):
    ...
    assert "维度" in text and "核心贡献" in text
