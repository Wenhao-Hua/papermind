...
def test_comparison_csv_header_and_rows():
    ...
    rows = list(reader)
    # First row: dimension column + one column per paper (by arxiv id)
    assert rows[0] == ["维度", "2307.08691", "1706.03762"]
    # All expected dimension labels are present in the first column
    labels = [r[0] for r in rows if r]
    for expected in ("标题", "arXiv", "年份", "核心贡献", "新颖之处", "关键方法", "性能/基准", "推荐硬件", "官方代码"):
        assert expected in labels
    # Cell values land in the right columns
...
def test_comparison_csv_with_synthesis():
    comp = _comparison()
    comp.synthesis = "Both papers address attention efficiency."
    text = to_csv(comp)
    reader = csv.reader(io.StringIO(text))
    rows = list(reader)
    # Last non-empty row should carry the synthesis
    non_empty = [r for r in rows if r]
    assert non_empty[-1][0] == "对比小结"
    assert non_empty[-1][1] == "Both papers address attention efficiency."
...
def test_comparison_to_csv_method_writes_file(tmp_path):
    ...
    assert "维度" in written and "2307.08691" in written
