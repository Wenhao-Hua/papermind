...
def test_comparison_csv_export():
    ...
    # one row per dimension (9 rows: 标题, arXiv, 年份, 核心贡献, 新颖之处, 关键方法, 性能/基准, 推荐硬件, 官方代码)
...
def test_comparison_csv_with_synthesis():
    comp = _comparison()
    comp = comp.model_copy(update={"synthesis": "Two great papers."})
    text = to_csv(comp)
    assert "对比小结" in text
    assert "Two great papers." in text
