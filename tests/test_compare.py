...
def test_comparison_csv_with_synthesis():
    comp = _comparison()
    comp.synthesis = "These two papers differ in speed."
    text = to_csv(comp)
    assert "对比小结" in text
    assert "These two papers differ in speed." in text
...
def test_comparison_csv_file_write(tmp_path):
    ...
    assert "faster attention" in written
