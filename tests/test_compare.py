...
def test_comparison_csv_via_method(tmp_path):
    ...
    assert "维度" in on_disk
    assert "2307.08691" in on_disk
...
def test_comparison_csv_with_synthesis():
    ...
    comp = Comparison(
        papers=[
            ComparedPaper(title="A", arxiv_id="1", year=2023, main_contribution="c1"),
            ComparedPaper(title="B", arxiv_id="2", year=2024, main_contribution="c2"),
        ],
        synthesis="A is older than B.",
    )
    text = to_csv(comp)
    assert "对比小结" in text
    assert "A is older than B." in text
