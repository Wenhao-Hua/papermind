...
def test_comparison_csv_structure():
    ...
    # dimension labels present as first column
...
def test_comparison_csv_with_synthesis():
    comp = _comparison()
    comp.synthesis = "Paper A excels at speed; Paper B introduced the paradigm."
    text = to_csv(comp)
    rows = list(csv.reader(io.StringIO(text)))
    synth_rows = [r for r in rows if r and r[0] == "对比小结"]
    assert synth_rows, "synthesis row should be present"
    assert "Paper A excels" in synth_rows[0][1]
